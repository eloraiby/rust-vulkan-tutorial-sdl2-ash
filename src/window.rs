...
impl ExampleBase {
    pub fn render_loop<F: Fn(&mut Self)>(&mut self, f: F) {
        'running: loop {
    ...
    pub fn new(window_width: u32, window_height: u32) -> Result<Self, Box<dyn Error>> {
        unsafe {
            ...
            let window = video_subsystem
                .window("Ash - SDL2 Example", 1024, 768)
                .vulkan()
                .build()
                .unwrap();
            ...
            for lp in &layers {
                println!(
                    " {}: {}",
                    String::from_utf8(lp.layer_name.iter().map(|x| *x as u8).collect()).unwrap(),
                    String::from_utf8(lp.description.iter().map(|x| *x as u8).collect()).unwrap()
                );
            }
            ...
            let mut ext_names = extension_names
                .iter()
            ...
            let instance_e = entry.create_instance(&create_info, None);
            println!("{:?}", instance_e.clone().err());
            let instance = instance_e.expect("Instance creation error");
